 ...
 */
public class CleanMenuBar extends MenuBar{
    ...
    public CleanMenuBar(){
        ...
        fileOptions[2].setOnAction((ActionEvent e) -> { //I'm feeling lucky/russian roulette mode/open random file
            ...
            try {
                BestTab.openImage(subFiles[rand.nextInt(subFiles.length)]);
            } catch (Exception ex) {
        ...
        fileOptions[fileOptions.length - 1].setOnAction((ActionEvent e) -> {
            Popups.createExitAlert(BestPaint.mainStage);
        });
        
        helpOptions[0].setOnAction((ActionEvent e) -> { //about window
            try {
                Popups.createTextWindow(new File(ABOUT_PATH), "About", 650, 350);
        ...
        helpOptions[1].setOnAction((ActionEvent e) -> {  //help window
            try {
                Popups.createTextWindow(new File(HELP_PATH), "Help", 650, 350);
            } catch (FileNotFoundException ex) {
                System.out.println(ex);
            }
        }); 
        helpOptions[2].setOnAction((ActionEvent e) -> {  //release notes
            try {
                Popups.createTextWindow(new File(REL_NOTES_PATH), "Release Notes", 635, 500);
            } catch (FileNotFoundException ex) {
                System.out.println(ex);
            }
        });
